...
void update(void)
{
	...
	if (fBTranslateFactor < 0.0f)
		fBTranslateFactor = fBTranslateFactor + 0.05f;
	else
	{
	...
	if (bHletterDisplayed)
	{
		if (fHTranslateFactor < 0.0f)
			fHTranslateFactor = fHTranslateFactor + 0.05f;
		else
			bA1letterDisplayed = True;
	}
	...
	if (bA1letterDisplayed)
	{
		if (fA1TranslateFactor < 0.0f)
			fA1TranslateFactor = fA1TranslateFactor + 0.05f;
		else
			bRletterDisplayed = True;
	}
	...
	if (bRletterDisplayed)
	{
		if (fRTranslateFactor > 0.0f)
			fRTranslateFactor = fRTranslateFactor - 0.05f;
		else
			bA2letterDisplayed = True;
	}
	...
	if (bA2letterDisplayed)
	{
		if (fA2TranslateFactor > 0.0f)
			fA2TranslateFactor = fA2TranslateFactor - 0.05f;
		else
			bTletterDisplayed = True;
	}
	...
	if (bTletterDisplayed)
	{
		if (fTTranslateFactor > 0.0f)
			fTTranslateFactor = fTTranslateFactor - 0.05f;
		else
			bJetEnabled = True;
	}
	...
	if(bJetEnabled)
	{
		...
		{
			angle = angle + 0.05f;
			fJetTranslateXFactor = fJetTranslateXFactorBegin - ( (fJetTranslateXFactorBegin + 10.5f) * cos(angle * 3.142 / 180));
		...
		if(fJetTranslateXFactor >=  -10.0f + (fLetterHalfWidth * 3.0f * 2.0f) -12.0f ) // -15f is offset to bring nose on the curve
		{
			bBletterTirangaDisplayed = True;
		}
		if(fJetTranslateXFactor >=  -10.0f + (fLetterHalfWidth * 3.0f * 3.0f) -12.0f) // -15f is offset to bring nose on the curve
		{
		...
		if(fJetTranslateXFactor >=  -10.0f + (fLetterHalfWidth * 3.0f * 8.0f) + 11.0f)// -15f is offset to bring nose on the curve 
		{
			...
			{
				angle2 = angle2 + 0.05f;
				fJetTranslateXFactor = -fJetTranslateXFactorBegin - ( (fJetTranslateXFactorBegin - 9.5f ) * cos(angle2 * 3.142 / 180));

...
BOOL gbAllObjectSynced = FALSE;

float fRotateLine = 0.0f;
float fRotateTriangle = 0.0f;
float fRotateCircle = .0f;

float fTranslateLine = 1.0f;
float fTranslateTriangle = 1.0f;
float fTranslateCircle = 1.0f;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		if(gbAllObjectSynced)
			PlaySound(MAKEINTRESOURCE(MYSONG), NULL, SND_RESOURCE | SND_ASYNC);
		break;
...
void display(void)
{
	...
	glTranslatef(-1.0f * fTranslateTriangle, -1.0f * fTranslateTriangle, -3.0f);
	glRotatef(fRotateTriangle, 0.0f, 1.0f, 0.0f);
	glLineWidth(3.0f);
	...
	// Matrix STACK concept
	glTranslatef(0.0f, fTranslateLine, -3.0f);
	...
	//Circle
	// Inside Circle Radius = Side / (2 * sqrt(3))
	GLdouble fRadius = (1.0F / (2.0F * sqrt(2.62F)));
	GLdouble fXcenter = 0.0f;
	GLdouble fYcenter = -0.5f + fRadius; //(1.0f / sqrt(3.0f));

	glLoadIdentity(); // Set Matrix to identity

	// Matrix STACK concept
	glTranslatef(fTranslateCircle, -1.0f * fTranslateCircle, -3.0f);
	glRotatef( fRotateCircle, 0.0f, 1.0f, 0.0f);
	glPointSize(3.0f);
	glBegin(GL_LINE_LOOP);
	for (GLdouble angle = 0.0f; angle <= (2.0f * 3.14f); angle += 0.01f)
	{
		GLdouble fXCord = fXcenter + (fRadius * cos(angle));
		GLdouble fYCord = fYcenter + (fRadius * sin(angle));
		glColor3f(1.0f, 1.0f, 0.0f);
		glVertex3d(fXCord, fYCord, 0.0f);
	}
	glEnd();
...
void update(void)
{
	//Code
	//When Annimation
	fRotateTriangle = fRotateTriangle + 0.1f;
	if (fRotateTriangle >= 360.0f)
		fRotateTriangle = 0.0f;

	fRotateCircle = fRotateCircle - 0.1f;
	if (fRotateCircle <= 0.0f)
		fRotateCircle = fRotateCircle + 360.0f;

	iFrameCounter++;
	if (iFrameCounter > 100)
	{
		if (fTranslateTriangle > 0.0f)
			fTranslateTriangle = fTranslateTriangle - 0.01f;
		if (fTranslateCircle > 0.0f)
			fTranslateCircle = fTranslateCircle - 0.01f;
		if (fTranslateLine > 0.0f)
			fTranslateLine = fTranslateLine - 0.01f;
		iFrameCounter = 0;
	}
	if (!gbAllObjectSynced && (fTranslateTriangle <= 0.0f) && (fTranslateCircle <= 0.0f) && (fTranslateLine <= 0.0f))
	{
		fTranslateTriangle = 0.0f;
		fTranslateCircle = 0.0f;
		fTranslateLine = 0.0f;

		gbAllObjectSynced = TRUE;
		PlaySound(MAKEINTRESOURCE(MYSONG), NULL, SND_RESOURCE | SND_ASYNC);
	}

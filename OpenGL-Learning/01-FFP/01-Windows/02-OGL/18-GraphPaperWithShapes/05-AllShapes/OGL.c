...
BOOL bGraphEnabled = TRUE;
BOOL bTriangleEnabled = TRUE;
BOOL bSquareEnabled = TRUE;
BOOL bCircleEnabled = TRUE;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		...
		case 't':
			if (bTriangleEnabled)
				bTriangleEnabled = FALSE;
			else
				bTriangleEnabled = TRUE;
			break;

		case 'S':
		case 's':
			if (bSquareEnabled)
				bSquareEnabled = FALSE;
			else
				bSquareEnabled = TRUE;
			break;
...
void display(void)
{
	...
	if(bGraphEnabled)
	{
		for (int i = -40; i <= 40; i++)
		{
			...
			glTranslatef(0.0f, (GLfloat)(i) * 0.025f, -3.0f);
		...
		for (int i = -40; i <= 40; i++)
		{
			...
			// Matrix STACK concept
			glTranslatef((GLfloat)(i) * 0.025f, 0.0f, -3.0f);
	...
	if (bTriangleEnabled)
	{
		...
		float fX = cos(210.0f * 2.0f * 3.14f / 360.0f) * 0.5f;
		float fY = sin(210.0f * 2.0f * 3.14f / 360.0f) * 0.5f;
		glVertex3f(fX, fY, 0.0f);

		fX = cos(330.0f * 2.0f * 3.14f / 360.0f) * 0.5f;
		fY = sin(330.0f * 2.0f * 3.14f / 360.0f) * 0.5f;
		glVertex3f(fX, fY, 0.0f);
	...
	if (bCircleEnabled)
	{
		...
		for (float angle = 0.0f; angle <= (2.0f * 3.14f); angle += 0.02f)
		{
			float fXCord = (fRadius * cos(angle));
			float fYCord = (fRadius * sin(angle));
			glColor3f(1.0f, 1.0f, 0.0f);

...
GLuint rimColorUniform = 0;
GLuint rimPowerUniform = 0;
GLuint rimLightPositionUniform = 0;
int rimColorcounter = 0;
...
GLfloat fRimLightPosition[] = {0.0f, 0.0f, -10.0f, 1.0f};
GLfloat fRimColor[] = {1.0f, 1.0f, 1.0f, 1.0f};
GLfloat fRimPower = 2.0f;
...
int initialize(void)
{
	...
	const GLchar* fragmentShaderSourceCode = 
					...
					"phongADSLight = phongADSLight + calculateRim(-normalizedTransformedNormals, -normalizedViewerVector);" \
					"}\n" \
					"else \n" \
					"{\n" \
					"vec3 normalizedTransformedNormals = normalize(oTransformedNormals); \n" \
	...
	rimLightPositionUniform = glGetUniformLocation(shaderProgramObject, "uRimLightPosition"); 
 	rimColorUniform = glGetUniformLocation(shaderProgramObject, "uRimColor");
	rimPowerUniform = glGetUniformLocation(shaderProgramObject, "uRimPower");
...
void display(void)
{
	...
	if(bLightingEnabled == TRUE)
	{
		...
		glUniform3fv(rimColorUniform, 1, fRimColor); 
		glUniform1f(rimPowerUniform, fRimPower);
	}
	else
	{
		glUniform3fv(rimColorUniform, 1, fRimColor); 

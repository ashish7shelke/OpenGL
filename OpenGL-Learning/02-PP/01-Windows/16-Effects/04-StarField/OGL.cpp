...
int initialize(void)
{
	...
	const GLchar* fragmentShaderSourceCode = 
		...
		"void main(void)\n" \
		"{\n" \
		"fragColor = starColor * texture(uTextureSampler, gl_PointCoord);\n" \
	...
	fractionTimeUniform = glGetUniformLocation(shaderProgramObject, "uTime");
	...
	glBufferData(GL_ARRAY_BUFFER, NUM_STARS * sizeof(vmath::vec3), NULL, GL_DYNAMIC_DRAW);
	...
	glBufferData(GL_ARRAY_BUFFER, NUM_STARS * sizeof(vmath::vec2), NULL, GL_DYNAMIC_DRAW);
	...
	if (bResult == FALSE)
	{
		fprintf(gpFile, "loadGlTexture(MY_SMILEY_BITMAP) failed .../n");
		return -1;
	}
...
void display(void)
{
    ...
    vmath::vec3* position = NULL;
    vmath::vec3* color= NULL;
	vmath::vec2* tex= NULL;

	position = new vmath::vec3[NUM_STARS];
	color = new vmath::vec3[NUM_STARS];
	tex = new vmath::vec2[NUM_STARS];

	for (int i = 0; i < NUM_STARS; i++)
	{
		float x = (random_float() * 2.0f - 1.0f) * 100.0f;
		float y = (random_float() * 2.0f - 1.0f) * 100.0f;
		float z = random_float();
		float r = 0.8f + random_float() * 0.2f;
		float g = 0.8f + random_float() * 0.2f;
		float b = 0.8f + random_float() * 0.2f;

		/*float x = (float)((rand()%50)-25.0f)*0.1f;
		float y = (float)((rand()%50)-25.0f)*0.1f;
		float z = (float)((rand()%50)-25.0f)*0.1f;
		float r = (float)((rand()%50)-25.0f)*0.02f;
		float g = (float)((rand()%50)-25.0f)*0.02f;
		float b = (float)((rand()%50)-25.0f)*0.02f;*/
		
		position[i] = vmath::vec3(x, y, z);
		color[i] = vmath::vec3(r, g, b);
		tex[i] = vmath::vec2(1.0f, 1.0f);
	}
	...
	glUniform1f(fractionTimeUniform, t);
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_position);
	glBufferData(GL_ARRAY_BUFFER, NUM_STARS * sizeof(vmath::vec3), position, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);

	glBindBuffer(GL_ARRAY_BUFFER, vbo_color);
	glBufferData(GL_ARRAY_BUFFER, NUM_STARS * sizeof(vmath::vec3), color, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);		

	/*glBindBuffer(GL_ARRAY_BUFFER, vbo_texture);
	glBufferData(GL_ARRAY_BUFFER, NUM_STARS * sizeof(vmath::vec2), tex, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);*/	

...
int initialize(void)
{
	...
	const GLchar* vertexShaderSourceCode = 
					...
					"for(int i = 0; i < 3; i++)\n" \
					"{\n" \
					"oLightDirection[i] = vec3(uLightPosition[i] - oEyeCoordinates); \n" \
					"}\n" \
	...
	const GLchar* fragmentShaderSourceCode = 
					...
					"void SpotLight(const in int i, const vec3 eye, const vec3 ecPosition3, const in vec3 normal, inout vec4 ambient, inout vec4 diffuse, inout vec4 specular) \n" \
					"{\n" \
						"float nDotVP;\n" \
						"float nDotHV;\n\n" \
						"float pf;\n" \
						...
						"vec4 ambientP  = vec4(1.0f, 0.0f, 1.0f, 1.0f);\n" \
						"vec4 diffuseP  = vec4(1.0f, 0.0f, 1.0f, 1.0f);\n" \
						"vec4 specularP = vec4(1.0f, 0.0f, 1.0f, 1.0f);\n" \
					...
					"void PointLight(const in int i, const vec3 eye, const vec3 ecPosition3, const in vec3 normal, inout vec4 ambient, inout vec4 diffuse, inout vec4 specular) \n " \
					"{\n" \
						"float nDotVP;\n" \
						"float nDotHV;\n" \
						"float pf;\n" \
						...
						"vec4 ambientP  = vec4(0.0f, 0.0f, 1.0f, 1.0f);\n" \
						"vec4 diffuseP  = vec4(0.0f, 0.0f, 1.0f, 1.0f);\n" \
						"vec4 specularP = vec4(0.0f, 0.0f, 1.0f, 1.0f);\n" \
					...
					"{\n" \
						...
						"vec4 ambientD = vec4(1.0f, 1.0f, 0.0f, 1.0f);\n" \
						"vec4 diffuseD  = vec4(1.0f, 1.0f, 0.0f, 1.0f);\n" \
						"vec4 specularD = vec4(1.0f, 1.0f, 0.0f, 1.0f);\n" \

						"nDotVP = max(0.0, dot(normal, normalize(vec3(oLightDirection[0]))));\n\n" \
					...
					"halfVector_directed_light = normalize(oLightDirection[0] -oViewerVector);\n" \
					"l_view_vector = normalize(-oViewerVector);\n" \
					"l_light_directD = normalize(oLightDirection[0]);\n" \
					
					"if (uLightSource == 0)\n" \
					"{\n"
						"DirectionalLight(1, oTransformedNormals, amb, diff, spec);\n" \
					"}\n" \
					"else if (uLightSource == 1) \n" \
					"{\n" \
						"PointLight(2, -oViewerVector, vec3(oEyeCoordinates), oTransformedNormals, amb, diff, spec);\n" \
					"}\n" \
					"else if (uLightSource == 2) \n" \
					"{\n" \
						"SpotLight(2, -oViewerVector, vec3(oEyeCoordinates), oTransformedNormals, amb, diff, spec);\n" \
					"}\n" \
					
					"//SpotLight(2, -oViewerVector, vec3(oEyeCoordinates), oTransformedNormals, amb, diff, spec);\n" \
					"light_final = amb + diff + spec;\n" \
					"FragColor = light_final;\n"
					"}\n" \
					"else\n" \
					"{" \
	...
	keyLightSourceUniform = glGetUniformLocation(shaderProgramObject, "uLightSource");
...
void display(void)
{
	...
	if(bLightingEnabled == TRUE)
	{
		...
		glUniform1i(keyLightSourceUniform, iLightSource);
	...
	else
	{
		...
		glUniform1i(keyLightSourceUniform, 0);
	}
